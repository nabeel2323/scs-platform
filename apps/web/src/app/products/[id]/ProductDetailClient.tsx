...
function galleryImages(product: ProductDetail): { src: string; alt: string; variantLabel?: string }[] {
  ...
  for (const m of media) {
    ...
    out.push({
      src,
      alt: variantLabel ? `${variantLabel} ΓÇö ${m.altText || product.title}` : (m.altText || product.title),
      variantLabel,
    });
  }
...
// ΓöÇΓöÇ Amazon-style gallery with mouse-follow zoom, thumbnail hover, counter ΓöÇΓöÇ
...
function ProductGallery({ images, title }: { images: { src: string; alt: string; variantLabel?: string }[]; title: string }) {
  ...
  if (!current) {
    return (
      <div className="pd-gallery-placeholder">
        <span style={{ fontSize: 72, opacity: 0.2 }} aria-label="No image">≡ƒôª</span>
      </div>
    );
  }
  ...
  return (
    <div onKeyDown={handleKeyDown} tabIndex={0} role="region" aria-label="Product image gallery" style={{ outline: 'none' }}>
      ...
      {/* Main image ΓÇö mouse-follow zoom (Amazon-style magnification) */}
      <div
        ref={mainRef}
      ...
      {/* Thumbnail strip ΓÇö hover previews + click to select */}
      {total > 1 && (
...
// ΓöÇΓöÇ Stock badge per variant ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Page ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
export default function ProductDetailClient() {
  ...
  // PHASE 7: dynamic variant selector ΓÇö the VariantSelector reports whether
  ...
  // PHASE 8: offer comparison table ΓÇö merges base offers + ranked analytics
  ...
  const handleAdd = useCallback(
    async (variantId: string, storeId: string, offerId?: string) => {
      ...
      try {
        ...
        // PHASE 13: distinguish per-offer added state so two "Added Γ£ô" badges
  ...
  return (
    ...
    <style>{`
      ...
      @media (max-width: 768px) {
        ...
        /* PHASE 8: offer comparison ΓÇö swap table for cards on mobile */
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div className="pd-detail-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 420px) 1fr', gap: 32, alignItems: 'start' }}>
        ...
        <div>
                ...
                <tbody>
                  {product.attributeValues.map(av => (
                    <tr key={av.code} style={{ borderBottom: `1px solid ${colors.border}` }}>
                      ...
                      <td style={{ padding: '6px 12px' }}>{av.value != null ? String(av.value) : 'ΓÇö'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {/* Variants ΓÇö PHASE 7: dynamic selector when product type has dimensions */}
          ...
          {/* Flat variant list ΓÇö shown when no product type or type has no variant dimensions */}
                      ...
                      <div key={v.id} style={{ padding: '12px 16px', background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: radii.md, opacity: outOfStock ? 0.75 : 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            ...
                            >
                              {outOfStock ? 'Out of Stock' : addedId === v.id ? 'Γ£ô Added' : 'Add to Cart'}
                            </button>
                          </div>
                        </div>
                        <div style={{ marginTop: 8, display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
                          {unit !== undefined ? (
                            ...
                            <span style={{ ...typeScale.bodySm, color: colors.warn }}>No active price for this item ΓÇö the cart will reject it until the seller publishes one</span>
                          )}
                          {tiers.length > 1 && (
          ...
          {/* PHASE 8: Offer comparison table ΓÇö replaces flat "Other Sellers" list */}
...
/* ΓöÇΓöÇ PHASE COS-13: Add to Compare button ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function CompareButton({ productId }: { productId: string }) {
  ...
  return (
    ...
    >
      {inList ? 'Γ£ô Comparing' : isFull ? 'Compare Full' : '+ Compare'}
    </button>
  );
}
