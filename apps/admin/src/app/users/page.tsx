    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1320 }}>
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: 0, letterSpacing: '-0.3px' }}>User Management</h1>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', margin: '6px 0 0' }}>All registered platform users — {total} total</p>
        </div>
      </div>
      ...
      <div style={{ padding: '28px 40px 48px', maxWidth: 1320 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: selectedUser ? '1fr 380px' : '1fr', gap: 20 }}>
          ...
          <div>
            {loading ? (
              ...
              <div style={tableWrap}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  ...
                  <tbody>
                    {users.map(u => (
                      <tr key={u.id} className="tbl-row" style={{ borderBottom: '1px solid #edf2f7', background: selectedUser?.id === u.id ? '#f0f7ff' : 'transparent' }}>
                        <td style={tdStyle}>
                        ...
                        <td style={tdStyle}>
                          <span style={pill(statusColor(u.status))}>{u.status}</span>
                        </td>
            ...
            {/* Pagination */}
            <TablePagination page={page} total={total} limit={limit} onPageChange={setPage} onLimitChange={l => { setLimit(l); setPage(0); }} />
          </div>

          {/* User detail panel */}
          {selectedUser && (
            <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: 20, position: 'sticky', top: 20, alignSelf: 'start', boxShadow: '0 1px 3px rgba(22,35,43,.06), 0 4px 14px rgba(22,35,43,.04)' }}>
                ...
                <>
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ marginTop: 8 }}>
                      <span style={pill(statusColor(selectedUser.status))}>{selectedUser.status}</span>
